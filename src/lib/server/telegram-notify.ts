...
/**
 * Deep link to the conversation; falls back to the app when the mail wasn't
 * stored as a message (unrouted). `/mail/:id` opens the whole thread from any
 * message in it, in either interface, so no thread lookup is needed.
 */
export function messageLink(appUrl?: string, emailId?: string | null): string | null {
	const base = appUrl?.trim().replace(/\/$/, '');
	if (!base) return null;
	return emailId ? `${base}/mail/${encodeURIComponent(emailId)}` : base;
}
