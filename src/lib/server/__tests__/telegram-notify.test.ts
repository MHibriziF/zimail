...
describe('buildTelegramMessage', () => {
	...
	test('links straight to the conversation when one is known', () => {
		const text = buildTelegramMessage(
			{ ...notification, emailId: 'email-1' },
			'https://mail.example.com/'
		);
		assert.match(text, /href="https:\/\/mail\.example\.com\/mail\/email-1"/);
	});

	test('omits the link when no app URL is configured', () => {
		assert.doesNotMatch(buildTelegramMessage({ ...notification, emailId: 'email-1' }), /<a href/);
	});
